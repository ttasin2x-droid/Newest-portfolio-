...
if (!localStorage.getItem('admin_bypass')) {
    ...
    // Check if we already logged this session to prevent duplicates on refresh
    if (!sessionStorage.getItem('logged_device')) {
        ...
        else if (/iPhone|iPad|iPod/i.test(ua)) deviceName = "iPhone/iOS";
        ...
        push(ref(db, 'visit_logs'), {
            device: deviceName,
            detail: ua, // Full user agent string if needed later
